import { ChevronRight, Fuel, Trash2 } from 'lucide-react';
...
type FuelStationPresetCardProps = {
...
export default function FuelStationPresetCard({
    location,
    fuelType,
    pricePerLiter,
    notes,
    ...
    const fuelLabel = fuelType ? fuelTypeMap[fuelType] ?? fuelType : 'Nav norādīts';
        ...
        <div className="flex w-full items-center justify-between rounded-2xl border border-[#d9ded9] bg-white p-5 text-left transition hover:border-[#c7d1ca] hover:shadow-sm">
            ...
            >
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#eef3ef]">
                    <Fuel className="h-5 w-5 text-[#166a4d]" />
                </div>
                ...
                <div className="min-w-0">
                    ...
                    <div className="mt-3 flex flex-wrap items-center gap-3 text-[14px] text-[#6b7a71]">
                        <span>Degviela: {fuelLabel}</span>

                        {pricePerLiter !== null && pricePerLiter !== undefined && (
                            <span>Cena/l: {pricePerLiter}</span>
                        )}
                    </div>
