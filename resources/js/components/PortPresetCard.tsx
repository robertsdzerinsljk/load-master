import { Anchor, ChevronRight, Trash2 } from 'lucide-react';

type PortPresetCardProps = {
    name: string;
    country: string;
    maxDraft?: string | number | null;
    cityDistanceKm?: string | number | null;
    loadingRateContainers?: string | number | null;
    loadingRateTons?: string | number | null;
    notes?: string | null;
    onClick?: () => void;
    onDelete?: () => void;
};

export default function PortPresetCard({
    name,
    country,
    maxDraft,
    cityDistanceKm,
        ...
        <div className="flex w-full items-center justify-between rounded-2xl border border-[#d9ded9] bg-white p-5 text-left transition hover:border-[#c7d1ca] hover:shadow-sm">
            ...
            >
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#eef3ef]">
                    <Anchor className="h-5 w-5 text-[#166a4d]" />
                </div>

                <div className="min-w-0">
                    <h3 className="text-[16px] font-semibold text-[#182219]">{name}</h3>
                    <p className="mt-1 text-[15px] text-[#5b6b61]">{country}</p>

                    <div className="mt-3 flex flex-wrap items-center gap-3 text-[14px] text-[#6b7a71]">
                        {maxDraft !== null && maxDraft !== undefined && (
                            <span>Maks. iegrimums: {maxDraft} m</span>
                        )}

                        {cityDistanceKm !== null && cityDistanceKm !== undefined && (
                            <span>Attālums līdz pilsētai: {cityDistanceKm} km</span>
                        )}

                        {loadingRateContainers !== null &&
                            loadingRateContainers !== undefined && (
                                <span>Konteineri/h: {loadingRateContainers}</span>
                            )}

                        {loadingRateTons !== null && loadingRateTons !== undefined && (
                            <span>t/h: {loadingRateTons}</span>
                        )}
                    </div>
